...
public interface ClientProtocol {
	 /**
     * When a client first connects to the server, it sends a hello message,
     * which includes some information about itself and the player:

       HELLO
       + DELIMITER + <supports chat>
       + DELIMITER + <supports challenge>
       + DELIMITER + <supports leaderboard>
       + DELIMITER + <player name>
       + EOC

     */
    void handleHandshake(boolean chat, boolean challenge, boolean leaderboard, String playerName)throws ServerUnavailableException;

    /**
     * After the handshake, clients should join one of three queues,
     * either for 2, 3 or 4-player games:
     ...
     * When the client has come up with a move, it sends it to the server:
       ...
       + DELIMITER + <player name>
       + DELIMITER + <direction>
       + DELIMITER + <index of field>  (multiple times)
       + EOC

     * All marbles involved in the move should be included, each separated by DELIMITER.
    ...
    /**
     * The client can always query the server for the current queue sizes,
